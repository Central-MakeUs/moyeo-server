...
public record PlaceViewResponse(
        ...
        @Schema(description = "추천 산출 방식입니다. MIDDLE_POINT는 좌표가 있으면 STRAIGHT_LINE_PREVIEW를, 좌표가 하나도 없으면 COORDINATES_PENDING을 반환합니다. RANDOM은 RANDOM_CATALOG_PREVIEW를 반환하고, 장소 조율 모임이 아니면 null입니다.", example = "STRAIGHT_LINE_PREVIEW", allowableValues = {"STRAIGHT_LINE_PREVIEW", "COORDINATES_PENDING", "RANDOM_CATALOG_PREVIEW"}, nullable = true, types = {"string", "null"})
        String recommendationBasis,

        @Schema(description = "참여자 출발지 좌표의 단순 평균 지점입니다. MIDDLE_POINT에서 출발지가 하나 이상 있을 때만 반환하고, RANDOM 또는 장소 조율 모임이 아니면 null입니다.", nullable = true, types = {"object", "null"})
    ...
    public record ParticipantDepartureResponse(
            ...
            @Schema(description = "출발지 주소. 입력하지 않았으면 null입니다.", example = "서울 강남구 테헤란로 123", nullable = true, types = {"string", "null"})
            String departureAddress,

            @Schema(description = "이동 수단입니다. 입력하지 않았으면 null입니다.", example = "PUBLIC_TRANSIT", allowableValues = {"PUBLIC_TRANSIT", "CAR"}, nullable = true, types = {"string", "null"})
    ...
    public record RecommendationResponse(
            ...
            @Schema(description = "참여자 출발지에서 상권까지의 평균 직선거리 미터. 랜덤 추천이면 null입니다.", example = "3200", nullable = true, types = {"integer", "null"})
            Long averageStraightDistanceMeters,

            @Schema(description = "정원이 찬 뒤 최초 조회에서 저장한 참여자 평균 실제 이동시간(초)입니다. 직선거리 미리보기에서는 null입니다.", example = "1200", nullable = true, types = {"integer", "null"})
            Long averageTravelTimeSeconds,

            @Schema(description = "정원이 찬 뒤 최초 조회에서 저장한 참여자 최대 실제 이동시간(초)입니다. 직선거리 미리보기에서는 null입니다.", example = "1800", nullable = true, types = {"integer", "null"})
            Long maxTravelTimeSeconds,

            @Schema(description = "상권과 매핑된 지하철역 정보입니다. 매핑이 없으면 null입니다. 거리·좌표는 노출하지 않습니다.", nullable = true, types = {"object", "null"})
