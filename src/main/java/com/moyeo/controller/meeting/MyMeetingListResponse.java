...
public record MyMeetingListResponse(
    ...
    public record Item(@Schema(description = "모임 ID", example = "17") Long meetingId,
                       @Schema(description = "모임 이름", example = "토요일 저녁 모임") String name,
                       @Schema(description = "커버 이미지 상대 API 경로. 없으면 null") String coverImageUrl,
                       @Schema(description = "내 역할", allowableValues = {"HOST", "MEMBER"}) String role,
                       ...
                       @Schema(description = "마감 시각. 마감이 없으면 null", example = "2026-07-29 13:54:43") @JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss") LocalDateTime deadlineAt,
