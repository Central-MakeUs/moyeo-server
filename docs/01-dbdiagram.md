...
- `commercial_areas` stores recommendation candidates independently from meetings. The seed contains 100 curated Seoul development/tourist-special areas and 20 curated Gyeonggi development areas. Seoul retains its source-owned code; Gyeonggi has no source code, so its `external_code` is a deterministic `GGD-` import identifier.
